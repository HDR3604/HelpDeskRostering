...
function normalize(s: ScheduleResponse): ScheduleResponse {
  return { ...s, assignments: Array.isArray(s.assignments) ? s.assignments : [] }
}
...
export async function updateSchedule(id: string, req: {
  title?: string
  assignments?: Assignment[]
}): Promise<ScheduleResponse> {
  const { data } = await apiClient.put<ScheduleResponse>(`/schedules/${id}`, req)
  return normalize(data)
}
