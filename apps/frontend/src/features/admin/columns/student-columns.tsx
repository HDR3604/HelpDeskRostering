...
export function getStudentColumns({
    ...
    return [
        ...
        },
        {
            id: 'programme',
            accessorFn: (row) =>
                `${row.transcript_metadata.current_programme} Year ${row.transcript_metadata.current_year}`,
            header: 'Programme',
            cell: ({ row }) => (
                <div>
                    <p className="text-sm">
                    ...
                    <p className="text-xs text-muted-foreground">
                        Year {row.original.transcript_metadata.current_year}
                    </p>
                </div>
            ),
        },
        {
            id: 'gpa',
            accessorFn: (row) => row.transcript_metadata.overall_gpa ?? 0,
