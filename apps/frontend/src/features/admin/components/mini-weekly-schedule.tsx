...
export function MiniWeeklySchedule({
    ...
    const hasOverflow = Object.values(byDay).some(
        (d) => d.length > MAX_VISIBLE_PER_DAY,
    )
                ...
                <div className="grid grid-cols-5 gap-2 transition-all duration-300 ease-in-out">
                    {WEEKDAYS_SHORT.map((day, idx) => {
                        ...
                        const visible = expanded
                            ? dayAssignments
                            : dayAssignments.slice(0, MAX_VISIBLE_PER_DAY)
                        const overflow =
                            dayAssignments.length - MAX_VISIBLE_PER_DAY
                                ...
                                {dayAssignments.length > 0 ? (
                                    <div className="space-y-1">
                                        {visible.map((a) => {
                                            ...
                                            const name =
                                                studentNames[a.assistant_id] ||
                                                a.assistant_id.slice(0, 6)
                                            const initials = name
                                                .split(' ')
                                                .map((n) => n[0])
                                                .join('')
