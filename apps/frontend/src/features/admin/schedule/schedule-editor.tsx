...
export function ScheduleEditor({
    ...
    const dateRange =
        ...
        (schedule.effective_to
            ? ` — ${formatDateMedium(schedule.effective_to)}`
            : ' onwards')
    ...
    const toolbarStats = useMemo(() => {
        ...
        const totalHours = Object.values(studentHours).reduce(
            (sum, h) => sum + h,
            0,
        )
    ...
    const studentAvailabilityMap = useMemo(
        () =>
            Object.fromEntries(
                students.map((s) => [String(s.student_id), s.availability]),
            ),
        [students],
    )
        ...
        (event: DragEndEvent) => {
            ...
            if (destId.startsWith('shift::')) {
                ...
                if (source.context === 'pool') {
                    ...
                    source.context === 'cell' &&
                    source.shiftId !== destShiftId
                ) {
                    dispatch({
                        type: 'MOVE_STUDENT',
                        fromShiftId: source.shiftId,
                        toShiftId: destShiftId,
                        studentId: source.studentId,
                    })
                    dropSucceededRef.current = true
                }
        ...
        <>
            ...
            <div className="flex flex-col items-center justify-center gap-4 py-20 text-center lg:hidden">
                ...
                <div>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                        The schedule editor requires a desktop or tablet in
                        landscape mode.
                    </p>
                </div>
            ...
            <div className="hidden lg:flex flex-col gap-5">
                ...
                >
                    ...
                    <DragOverlay
                        dropAnimation={
                            dropSucceededRef.current ? null : undefined
                        }
                    >
                        {activeDragData && (
                            <StudentChipOverlay
                                name={activeDragData.name}
                                colorIndex={activeDragData.colorIndex}
                            />
                        )}
                    </DragOverlay>
                </DndContext>
