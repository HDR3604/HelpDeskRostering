...
export function CreateScheduleDialog({
    ...
    const [formValues, setFormValues] = useState<CreateScheduleValues | null>(
        null,
    )
                ...
                {generationId ? (
                    ...
                    <>
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                Set up a new weekly schedule with selected
                                students.
                            </DialogDescription>
                        </DialogHeader>
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormControl>
                                                <Input
                                                    placeholder="e.g. Week 6 — Feb 24-28 Schedule"
                                                    {...field}
                                                />
                                            </FormControl>
                                            <FormMessage />
                                        ...
                                        <FormItem>
                                            ...
                                            {selectedConfig && (
                                                <p className="text-xs text-muted-foreground">
                                                    Targets{' '}
                                                    {
                                                        selectedConfig.baseline_hours_target
                                                    }
                                                    h/student
                                                    {selectedConfig.solver_time_limit !=
                                                        null &&
                                                        ` · ${selectedConfig.solver_time_limit}s time limit`}
                                                </p>
                                            )}
                                            <FormMessage />
                                        </FormItem>
...
function GeneratingView({
    ...
    const isFailed =
        status?.status === 'failed' || status?.status === 'infeasible'
                        ...
                        <>
                            <span className="text-sm font-medium">
                                {status?.status === 'pending'
                                    ? 'Queued...'
                                    : getStepLabel(status?.progress ?? 0)}
                            </span>
                ...
                {isFailed && status?.error_message && (
                    <div className="flex w-full flex-col gap-2 rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2.5">
                        ...
                        <p className="text-xs text-muted-foreground pl-6">
                            Try adjusting students or constraints.
                        </p>
                    </div>
                )}
            </div>
