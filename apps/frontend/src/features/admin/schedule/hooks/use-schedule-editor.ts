    ...
    switch (action.type) {
        case 'ASSIGN_STUDENT': {
            ...
            const next = {
                ...
                [action.shiftId]: [...current, action.studentId],
            }
            return {
                ...state,
                assignmentsByShift: next,
                isDirty: isDirty(next, state.originalAssignments),
            }
        }
        ...
        case 'MOVE_STUDENT': {
            const fromList = (
                state.assignmentsByShift[action.fromShiftId] ?? []
            ).filter((id) => id !== action.studentId)
            const toList = state.assignmentsByShift[action.toShiftId] ?? []
            if (toList.includes(action.studentId)) return state
            const next = {
                ...state.assignmentsByShift,
                [action.fromShiftId]: fromList,
                [action.toShiftId]: [...toList, action.studentId],
            }
            return {
                ...state,
                assignmentsByShift: next,
        ...
        case 'SET_GENERATED': {
            return {
                ...state,
                assignmentsByShift: action.assignmentsByShift,
                isDirty: isDirty(
                    action.assignmentsByShift,
        ...
        case 'MARK_SAVED':
            return {
                ...state,
                originalAssignments: structuredClone(state.assignmentsByShift),
                isDirty: false,
                isSaving: false,
            }
        default:
...
export function useScheduleEditor(
    ...
    const [state, dispatch] = useReducer(
        editorReducer,
        initializeState(schedule, includedStudentIds),
    )
    ...
    const unassignedStudents = useMemo(
        () =>
            students.filter(
                (s) => !assignedStudentIds.has(String(s.student_id)),
            ),
    ...
    const handleGenerate = useCallback(async () => {
        ...
        toast.success('Schedule auto-generated', {
            description: 'Review assignments and save when ready.',
        })
