...
func (s *SchedulerService) GenerateSchedule(req types.GenerateScheduleRequest) (*types.GenerateScheduleResponse, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("%w: %w", types.ErrMarshalRequest, err)
	}
	...
	if scheduleResponse.StatusCode != http.StatusCreated {
		...
		case scheduleResponse.StatusCode == http.StatusUnprocessableEntity:
			return nil, fmt.Errorf("%w: %s", types.ErrInvalidRequest, string(body))
		case scheduleResponse.StatusCode >= 500:
			return nil, fmt.Errorf("%w: status %d", types.ErrSchedulerInternal, scheduleResponse.StatusCode)
		default:
			return nil, fmt.Errorf("%w: unexpected status %d: %s", types.ErrSchedulerInternal, scheduleResponse.StatusCode, string(body))
		}
	}
	...
	if err := json.NewDecoder(scheduleResponse.Body).Decode(&result); err != nil {
		...
		return nil, fmt.Errorf("%w: %w", types.ErrUnmarshalResponse, err)
	}
