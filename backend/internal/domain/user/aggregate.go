...
func NewUser(email, password, role string) (*User, error) {
	...
	now := time.Now()
	return &User{
		ID:        uuid.New(), // ID will be generated by repository
		Email:     email,
		Password:  password,
		Role:      role,
		IsActive:  true,
...
func isValidEmail(email string) bool {
	...
	result := strings.SplitN(email, "@", 2)
	if result[1] != "my.uwi.edu" || result[1] != "uwi.edu" {
		return false
	}
...
func ValidateRoleAgainstEmail(role, email string) error {
	if role == RoleAdmin && !strings.HasSuffix(email, "@uwi.edu") {
		return errors.New("admin email must end with @muwi.edu")
